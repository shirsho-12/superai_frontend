...
export const useComplianceAPI = () => {
  ...
  const generateAmendments = useCallback(async (gapId: string) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Amendment generation failed"
      );
      throw err;
    } finally {
      setLoading(false);
  ...
  const analyzeCrossImpact = useCallback(
    async (documentIds: string[], policyIds: string[]) => {
      setLoading(true);
      setError(null);
      try {
        const result = await apiProvider.analyzeCrossImpact(
          documentIds,
          policyIds
        );
        return result;
        ...
        setError(
          err instanceof Error ? err.message : "Cross-impact analysis failed"
        );
        throw err;
      } finally {
        setLoading(false);
  ...
  const getUploadedDocuments = useCallback(
    async (filters?: { tag?: string; dateFrom?: string; dateTo?: string }) => {
      setLoading(true);
      setError(null);
      try {
